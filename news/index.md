...
- Confidence intervals follow one argument convention throughout
  (`conf.level`, `sides`, `method`), across proportions, counts,
  location, scale and correlation.
- [`bootCI()`](../reference/bootCI.md) provides bootstrap intervals for
  arbitrary statistics; the resampling is implemented in C++ via Rcpp,
  RcppArmadillo and RcppParallel.
- [`postHoc()`](../reference/postHoc.md) collects the post-hoc
  procedures for multiple comparisons behind one interface, with a plot
  method for the resulting intervals.
