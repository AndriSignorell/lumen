...
test_that("dunnTest: airquality formula example runs without error", {
  expect_no_error({
    res <- dunnTest(Ozone ~ factor(Month), data = airquality, na.action = na.omit)
    expect_s3_class(res, "rankTest")
...
test_that("dunnTest.default: two.sided equals 2 * min(one-sided)", {
  set.seed(2)
...
test_that("dunnTest.default: alpha controls the significance label", {
  set.seed(1)
  x <- c(rnorm(20, 0), rnorm(20, 5))
  g <- rep(c("A", "B"), each = 20)
