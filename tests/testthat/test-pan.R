...
# pan_cpp(A, M, C, N) computes P(sum_i lambda_i * chi^2_1 <= x)
...
# Helper: build A vector as expected by pan_cpp()
...
test_that("pan: returns value in [0, 1]", {
  eigs <- c(3, 2, 1)
  res  <- pan_cpp(make_A(3.0, eigs), M = 3, C = 0, N = 20)
  expect_gte(res, 0)
  expect_lte(res, 1)
})
...
test_that("pan: very large x gives p close to 1", {
  eigs <- c(3, 2, 1)
  res  <- pan_cpp(make_A(1e6, eigs), M = 3, C = 0, N = 20)
  expect_gt(res, 0.99)
})
...
test_that("pan: monotone in x", {
  eigs <- c(3, 2, 1)
  xs   <- c(0, 1, 3, 6, 10)
  ps   <- sapply(xs, function(x) pan_cpp(make_A(x, eigs), M = 3, C = 0, N = 20))
...
test_that("pan: increasing eigenvalues give monotone CDF", {
  eigs <- c(1.0, 2.0, 3.0)
  xs   <- c(1, 3, 6, 10, 20)
  ps   <- sapply(xs, function(x) pan_cpp(make_A(x, eigs), M = 3, C = 0, N = 30))
  expect_true(all(diff(ps) >= 0))
})
...
test_that("pan: larger sum of eigenvalues shifts CDF right", {
  ...
  r_small <- pan_cpp(make_A(q, eigs_small), M = 3, C = 0, N = 30)
  r_large <- pan_cpp(make_A(q, eigs_large), M = 3, C = 0, N = 30)
  expect_gt(r_small, r_large)
})

test_that("pan: N=1 still returns value in [0,1]", {
  eigs <- c(2, 1)
  res  <- pan_cpp(make_A(2.0, eigs), M = 2, C = 0, N = 1)
  expect_gte(res, 0)
  expect_lte(res, 1)
})
...
test_that("pan: larger N gives more accurate result", {
  ...
  eigs <- rep(lam, k)
  r10  <- abs(pan_cpp(make_A(q, eigs), M = k, C = 0, N = 10)  - ref)
  r100 <- abs(pan_cpp(make_A(q, eigs), M = k, C = 0, N = 100) - ref)
