...
test_that("symmetry for uniform Dirichlet", {
  ...
  p1 <- pdirichlet(q1, alpha, R = 2e4)
  p2 <- pdirichlet(q2, alpha, R = 2e4)
...
test_that("invalid input throws error", {
  expect_error(
    pdirichlet(c(0.2,0.8), c(1,1,1), R = 1000)
  )
})
...
test_that("agrees with R Monte Carlo implementation", {
  ...
  pdirichlet_R <- function(q, alpha, R = 1e4) {
    sims <- rdirichlet(R, alpha)
  ...
  p_r   <- pdirichlet_R(q, alpha, R = 2e4)
