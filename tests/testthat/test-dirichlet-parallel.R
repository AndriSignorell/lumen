...
test_that("symmetry for uniform Dirichlet", {
  ...
  p1 <- pdirichlet(q1, alpha, nSim = 2e4)
  p2 <- pdirichlet(q2, alpha, nSim = 2e4)
...
test_that("CDF is monotone in q", {
  ...
  p_small <- pdirichlet(q_small, alpha, nSim = 2e4)
  p_large <- pdirichlet(q_large, alpha, nSim = 2e4)
...
test_that("invalid input throws error", {
  expect_error(
    pdirichlet(c(0.2,0.8), c(1,1,1), nSim = 1000)
  )
})
...
test_that("agrees with R Monte Carlo implementation", {
  ...
  pdirichlet_R <- function(q, alpha, nSim = 1e4) {
    sims <- rdirichlet(nSim, alpha)
    mean(apply(sims <= matrix(q, nrow = nSim, ncol = length(q), byrow = TRUE), 1, all))
  }
  
  q <- c(0.2, 0.3, 0.5)
  alpha <- c(1,1,1)
  
  p_cpp <- pdirichlet(q, alpha, nSim = 2e4)
  p_r   <- pdirichlet_R(q, alpha, nSim = 2e4)
