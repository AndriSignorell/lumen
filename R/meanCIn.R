...
#' the mean depends recursively on the sample size, as the sample size defines
...
meanCIn <- function(ci, sd, interval=c(2, 1e5), conf.level=0.95, norm=FALSE,
  ...
  if(width > sd){
    warning("Width of confidence interval > 2*sd, sample size n=1 suffices for this case.")
    return(1)
