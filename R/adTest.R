...
#' Adrian Baddeley, previously published in the \pkg{goftest} package.
#' Rewritten in C++ with an adapted R interface to conform to package
#' standards.
#'
...
andersonDarlingTest <- function(x, null = "punif", ..., estimated = FALSE,
  ...
  if (estimated && n <= 4) {
    warning("too few observations for Braun's adjustment, ",
            "performing the unadjusted test")
    estimated <- FALSE
  }
  ...
  # describe the parameters passed to the null distribution function
  extras <- list(...)
  parnames <- intersect(names(extras), names(formals(F0)))
  pard <- if (length(parnames) > 0)
    paste("with", ngettext(length(parnames), "parameter", "parameters"),
          paste(parnames, "=",
                vapply(extras[parnames], paste, "", collapse = " "),
                collapse = ", "))

  coda <- if (estimated) "parameters estimated from data" else
    "parameters fixed"

  # note: print.htest() passes method through strwrap(), which collapses
  # newlines, so the components are joined into a single sentence
  METHOD <- paste(
    c("Anderson-Darling test of goodness-of-fit",
      ADJUST,
...
braun <- function(U, simpletest, m) {
  ...
  n <- length(U)
  if (n < 2 * m) stop("Insufficient data for Braun's method")
  ...
  # combine
  list(statistic = max(statistics),
       pvalue    = 1 - (1 - min(pvalues))^m,
       statname  = paste0(zz[[1]]$statname, "max"))
}
