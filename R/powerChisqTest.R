...
powerChisqTest <- function (n = NULL, effectSize = NULL, df = NULL, sig.level = 0.05, power = NULL) {
  ...
  p.body <- quote({
    ...
    pchisq(k, df = df, ncp = n * effectSize^2, lower = FALSE)
  })
  if (is.null(power))
    power <- eval(p.body)
  else if (is.null(effectSize))
    effectSize <- uniroot(function(effectSize) eval(p.body) - power, c(1e-10, 1e+05))$root
  else if (is.null(n))
  ...
  structure(list(effectSize = effectSize, n = n, df = df, sig.level = sig.level,
